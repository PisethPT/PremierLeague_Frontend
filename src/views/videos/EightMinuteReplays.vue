...
const allEightMinuteReplays = ref(10);
const startYearOfEightMinuteReplays = ref(2024);
...
<template>
    <!-- 2016/17 - 2024/25 -->
    <div v-for="(years, index) in allEightMinuteReplays" :key="index"
        class="flex flex-col gap-5 !bg-[#28002b] rounded-2xl w-full p-4">
        <div class="flex justify-between items-center">
            <span class="text-white text-2xl font-bold">{{ startYearOfEightMinuteReplays - index }}/{{
                ((startYearOfEightMinuteReplays - index) + 1).toString().slice(2) }}</span>
